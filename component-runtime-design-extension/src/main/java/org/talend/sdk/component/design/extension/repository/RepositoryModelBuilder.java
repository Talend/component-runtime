...
public class RepositoryModelBuilder {
    ...
    public RepositoryModel create(final ComponentManager.AllServices services,
        ...
        return new RepositoryModel(familyMetas
                ...
                .map(familyMeta -> Stream
                        ...
                        .map(p -> createConfig(familyMeta.getPlugin(), familyMeta.getPluginLocation(), services, p,
                                familyMeta.getName(), familyMeta.getIcon(), migrationHandlerFactory))
    ...
    private Config createConfig(final String plugin, final String pluginLocation,
            final ComponentManager.AllServices services, final ParameterMeta config, final String familyName,
            final String familyIcon, final MigrationHandlerFactory migrationHandlerFactory) {
        final Config c = new Config();
        c.setIcon(familyIcon);
        c.setKey(getKey(familyName, config.getMetadata()));
        c.setMeta(config);
        c.setId(IdGenerator.get(plugin, c.getKey().getFamily(), c.getKey().getConfigType(), c.getKey().getConfigName(),
                pluginLocation));
