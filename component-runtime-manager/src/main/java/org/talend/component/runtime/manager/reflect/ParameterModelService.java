 ...
 */
package org.talend.component.runtime.manager.reflect;
...
public class ParameterModelService {
    ...
    public ParameterModelService() {
        enrichers = StreamSupport
                .stream(Spliterators.spliteratorUnknownSize(ServiceLoader.load(ParameterExtensionEnricher.class).iterator(),
    ...
    public List<ParameterMeta> buildParameterMetas(final Executable executable, final String i18nPackage) {
        return Stream.of(executable.getParameters()).filter(p -> !p.getType().isAnnotationPresent(Service.class)
                && !p.getType().getName().startsWith("org.talend.component.api.service.")).map(parameter -> {
    ...
    private ParameterMeta buildParameter(final String name, final String prefix, final Type genericType,
        ...
        switch (type) {
        case OBJECT:
            final List<ParameterMeta> meta = buildParametersMetas(name, normalizedPrefix + ".", genericType, annotations,
                    i18nPackage);
    ...
    private Map<String, String> buildExtensions(final String name, final Type genericType, final Annotation[] annotations) {
