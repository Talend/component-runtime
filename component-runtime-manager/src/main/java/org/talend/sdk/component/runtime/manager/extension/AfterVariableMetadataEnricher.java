...
public class AfterVariableMetadataEnricher implements ComponentMetadataEnricher {
    ...
    private static final Set<Class<? extends Annotation>> SUPPORTED_ANNOTATIONS = new HashSet<>();

    private static final String META_KEY_AFTER_VARIABLE = "variables::after::value";
    ...
    static {
        SUPPORTED_ANNOTATIONS.add(PartitionMapper.class);
        SUPPORTED_ANNOTATIONS.add(Emitter.class);
        SUPPORTED_ANNOTATIONS.add(Output.class);
    }

    @Override
    public Map<String, String> onComponent(final Type type, final Annotation[] annotations) {
        boolean noneMatch = Stream.of(annotations).map(Annotation::getClass).noneMatch(SUPPORTED_ANNOTATIONS::contains);
        if (!noneMatch) {
