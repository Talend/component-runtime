...
public class SvgIconResolver {
    ...
    public Icon resolve(final ClassLoader loader, final String icon) {
        ...
        return Stream.of("%s.svg", "icons/%s.svg", icon + "_icon32.png", "icons/" + icon + "_icon32.png")
                .map(pattern -> String.format(pattern, icon)).map(path -> {
                    final InputStream resource = loader.getResourceAsStream(path);
                    if (resource == null) {
                        return null;
                    }
                    return new Icon(path.endsWith("svg") ? MediaType.APPLICATION_SVG_XML : "image/png", toBytes(resource));
