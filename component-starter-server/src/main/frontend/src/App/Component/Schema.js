		...
		if (!!this.state.edited) {
			...
			nodeView = (
				<span>
					...
					{!this.props.readOnly && (
						<Icon className={theme.nodeIcon} name="talend-trash" onClick={this.deleteNode} />
					)}
					{!this.props.parent && (
						<Help
							...
							content={
								<span>
									...
									<p>
										You can click on the <Icon name="talend-plus-circle" /> to add an object field,
										on the <Icon name="talend-trash" /> to delete one and on any field to edit its
										name and type.
									</p>
									<p>
										To add a nested object just add a field, click on it, customize its name and
										select the type object into the select box then valid your updates.
									</p>
									<p>
										<Icon name="talend-info-circle" /> The field names must be a valid java name (no
										space, special characters, ...). If the field name contains some specific
										keywords it will be considered as a credential and marked as such (for instance{' '}
			...
			<div>
				<span>
					{!this.props.node.type && (
						<Icon
							className={theme.nodeIcon}
							name={nodeIcon}
							onClick={() => this.setState({ opened: !this.state.opened })}
						/>
					)}
					<nodeLabel>{nodeView}</nodeLabel>
				</span>
