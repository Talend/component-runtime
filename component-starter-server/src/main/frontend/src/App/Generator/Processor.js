...
class Connection extends React.Component {
	constructor(props) {
		...
		['switchStructureType', 'showDrawer'].forEach(i => (this[i] = this[i].bind(this)));
	}
		...
		this.props.onUpdateDrawers([
			...
			>
				...
				<EmbeddableToggle
					...
					onChange={this.switchStructureType}
				/>
			</Drawer>,
		]);
	}
			...
			<li className={this.props.theme.Connection}>
				<div>
					<span onClick={this.showDrawer}>
						...
						<span className={this.props.theme.recordType}>
		...
		return (
			<ul className={this.props.theme.Connections}>
				...
				{(!this.props.connections || this.props.connections.length === 0) && (
					<li className={this.props.theme.ConnectionEnd} />
				)}
			</ul>
		);
	}
