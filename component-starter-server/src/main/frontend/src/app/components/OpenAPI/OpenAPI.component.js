...
class EndpointItem extends React.Component {
    ...
    onChange(event) {
        this.props.onChange({ endpoint: this.props, checked: !!event.target.checked, project: this.props.project });
    }
            ...
            <div className="form-group">
                <div className="checkbox">
                    <label>
                        ...
                        <span>{this.props.operationId}: {this.props.verb} {this.props.path}</span>
                    </label>
                </div>
            </div>
        );
...
function EndpointsSelector(props) {
    return (
        <form className={theme.formPadding} noValidate onSubmit={() => false}>
            <h2>
                ...
                <Help
                    ...
                    content={
                        <div>
                            <p>
                                The right editor enables you to edit the OpenAPI document.
                            </p>
                            <p>
                                The left list will list the available endpoints and let you select the ones you want to handle in your generated component.
                            </p>
                        </div>
                    }
                />
            ...
            {props.items && props.items.map((it, idx) => (<EndpointItem key={it.operationId} {...it} onChange={props.onUpdate} project={props.project} />))}
        </form>
    );
}
...
class OpenAPI extends React.Component {
    ...
    onEndpointUpdate({ endpoint, checked, project }) {
        const endpoints = project.$$openapi.endpoints.map(it => it.operationId == endpoint.operationId ? { ...it, checked } : it);
        project.$$openapi = {
            ...project.$$openapi,
            endpoints,
            selectedEndpoints: endpoints.filter(it => it.checked),
    ...
    render() {
        return (
            <ProjectContext.Consumer>
				{ project => {
                    if (!project.$$openapi) { // note: we could use an openapicontext here
            ...
            <div className={generatorTheme.Generator}>
                <div className={generatorTheme.container}>
                    ...
                    <div className={generatorTheme.content}>
                        <main>
                            <Switch>
                                <Route exact path="/openapi/project" component={(props) => <ProjectMetadata hideFacets={true} hideCategory={true} />} />
                                <Route exact path="/openapi/design" component={OpenAPITrans} />
                                <Route exact path="/openapi/export" render={(props) => <Finish {...props} openapi={true} actionUrl={GENERATOR_OPENAPI_ZIP_URL} />} />
                            </Switch>
                        </main>
                    </div>
                </div>
