    ...
    private ProjectRequest toRequest(final ProjectModel model) {
        ...
        return new ProjectRequest(
                ...
                ofNullable(model.getSources())
                        .map(s -> s.stream()
                                   .map(i -> new ProjectRequest.SourceConfiguration(i.getName(), i.isGenericOutput(), toStructure(i.getOutputStructure())))
                                   .collect(toList()))
                        .orElse(emptyList()),
                ofNullable(model.getProcessors())
                        .map(s -> s.stream().map(i -> new ProjectRequest.ProcessorConfiguration(i.getName(), i.isGenericInputs(), i.isGenericOutputs(),
                                i.getInputs(), i.getOutputs(),
                                ofNullable(i.getInputStructures()).map(is -> is.stream().map(this::toStructure).collect(toList())).orElse(emptyList()),
                                ofNullable(i.getOutputStructures()).map(is -> is.stream().map(this::toStructure).collect(toList())).orElse(emptyList())))
                                   .collect(toList()))
                        .orElse(emptyList()));
    }

    private ProjectRequest.DataStructure toStructure(final ProjectModel.Model model) {
        return new ProjectRequest.DataStructure(model.getEntries() == null ? emptyList() : model.getEntries().stream()
            .map(e -> new ProjectRequest.Entry(e.getName(), e.getType())).collect(toList()));
    }
}
