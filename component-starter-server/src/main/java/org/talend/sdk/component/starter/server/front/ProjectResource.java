...
public class ProjectResource {
    ...
    private ProjectRequest toRequest(final ProjectModel model) {
        ...
        return new ProjectRequest(
            ofNullable(model.getBuildType()).orElse(
                "maven").toLowerCase(Locale.ENGLISH),
            ...
            ofNullable(
                model.getPackageBase()).orElse("com.application").replace('/', '.'),
            ...
            ofNullable(
                model.getSources())
                    .map(
                        s -> s.stream()
                            .map(
                                i -> new ProjectRequest.SourceConfiguration(i.getName(), i.getIcon(), i.isStream(),
                                    toStructure(false,
                                        i.getConfigurationStructure()).getStructure(),
                ...
                .map(i -> new ProjectRequest.ProcessorConfiguration(i.getName(), i.getIcon(),
                    ...
                    ofNullable(i.getInputStructures())
                        .map(is -> is.stream().collect(
                            toMap(n -> unifiedName(n.getName()), nm -> toStructure(nm.isGeneric(), nm.getStructure()))))
                        .orElse(emptyMap()),
    ...
    private ProjectRequest.StructureConfiguration toStructure(final boolean generic, final ProjectModel.Model model) {
        return new ProjectRequest.StructureConfiguration(!generic ? new ProjectRequest.DataStructure(
            model == null || model.getEntries() == null ? emptyList() : model.getEntries().stream()
