...
public class TalendComponentKitTesting implements FacetGenerator, Versions {
    ...
    private Stream<InMemoryFile> createSourceTest(final String testJava, final String packageBase,
        ...
        return sources.stream().flatMap(source -> {
            ...
            final boolean hasConfig = source.getConfiguration() != null
                && source.getConfiguration().getEntries() != null && !source.getConfiguration().getEntries().isEmpty();
        ...
        return processors.stream().flatMap(processor -> {
            ...
            final Set<Map.Entry<String, String>> inputBranches =
                processor.getInputStructures().entrySet().stream().flatMap(in -> {
                    final String inName = in.getValue().isGeneric() ? "ObjectMap" : capitalize(processor.getName())
            ...
            final Set<Map.Entry<String, String>> outputBranches =
                !isOutput ? processor.getOutputStructures().entrySet().stream().flatMap(e -> {
                    final String outName = e.getValue().isGeneric() ? "ObjectMap" : capitalize(processor.getName())
                        + capitalize(names.sanitizeConnectionName(e.getKey())) + "Output";
                    Map<String, String> map = new HashMap<String, String>() {
