...
Mark a method as returning a database mapping from a connector configuration and some other parameters. Use this annotation if database mapping can be dynamic and `@DatabaseMapping.Mapping` is set to `custom`. The functionality is for the Studio only.
