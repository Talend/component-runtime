...
TIP: always prefer using light validations when possible.
...
The validations are triggers based on "events" so if you mark part of a configuration as `@Validable` but it is translated
to a widget without any interaction then nothing will never happen. The rule of thumb here is to ensure you mark only
