...
image:mvn_clean_install.png[Maven clean install,70%,window="_blank",link="_images/mvn_clean_install.png"]
...
This goal named `scan-descriptor` will scan the current module - optional other configured folders - to precompute the list
of interesting classes for the framework (components, services). This saves some bootstrap time when launching a job which
is interesting for some execution cases:
...
Configuration - excluding the ones used for defaults only:
...
|scannedDirectories|Explicit list of directories to scan.|`talend.scan.scannedDirectories`| If not set default to `${project.build.outputDirectory}`
|scannedDependencies|Explicit list of dependencies to scan - set them in format `groupId:artifactId`. It is appended to the file to scan.|`talend.scan.scannedDependencies`| -
|===
