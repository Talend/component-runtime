...
|validateDataSet| Ensures that any `@DataSet` has a unique name. Also ensures there is a source instantiable just filling the dataset properties (all others not being required).|`talend.validation.dataset`| true
