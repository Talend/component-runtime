...
image:mvn_clean_install.png[Maven clean install,window="_blank",link=self,70%]
...
To render the generated documentation in HTML or PDF, you can use the Asciidoctor Maven plugin. You can configure both executions if you want both HTML and PDF renderings.
...
|type|Specifies the type: `connector' or server `extension'|talend.car.type|connector
|===
