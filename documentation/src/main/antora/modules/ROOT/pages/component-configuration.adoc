...
to not have `arg0`, `arg1`, ... as names). It is highly recommanded to use a hava friendly name and it is forbidden to include `.` character in
it. Also note that starting with a `$` is forbidden too.
...
|===
| Option name|Valid
| myName | icon:check-circle[role="green"]
| my_name | icon:check-circle[role="green"]
| my.name | icon:times-circle[role="red"]
| $myName | icon:times-circle[role="red"]
|===
