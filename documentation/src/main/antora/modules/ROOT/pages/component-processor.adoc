...
----
== Support show dynamic output with a condition
...
* The return type of a @AvailableOutputFlows service is a List<String>, the build should fail if not (I think can be

* A TCK validator should check that, for processors with @ConditionalOutputFlows the related  service
@AvailableOutputFlow must expect only 1 parameter that has the same type of the processor configuration.
...
public class MyProcessor implements Serializable {
  ...
  public void process(@Input final Record input,
          ...
          @Output("second_flows") final OutputEmitter<Record> second,
          @Output("third_flows") final OutputEmitter<Record> third) {
      [.........]
  }
}
 ...
 @AvailableOutputFlows("my_service_that_return_active_flows")
  ...
  public Collection<String> getAvailableFlows(@Option("configuration") Config config) {
      List<String> flows = Arrays.asList("__default__", "second_flows");
      if (config.isABoolean()) {
          flows.add("third_flows");
      }
      return flows;
  }
