...
You can use a custom key with the `custom()` method but the icon may not be rendered properly. The icon defaults to *Star*. +
You can check the link:icons.html[icon gallery] to check out the available icons before creating your own. +
