...
Browse the API description using xref:rest-swagger.adoc[OpenAPI].
...
Note that Component Server adds to these default Talend profiles the profile `KAFKA` for a mode where all logs are sent to kafka.

TIP: if you want the exact configuration you can check our `component-runtime/images/component-server-image/src/main/resources` folder.

==== default or TEXT profile

The console logging is on at `INFO` level by default and you can customize it by setting `CONSOLE_LOG_LEVEL` environment variable to `DEBUG`, `INFO`, `WARN` or any other log level supported by log4j2.
...
1. logs on the console using `CONSOLE_LOG_LEVEL` configuration as in default profile. It uses the formatting you will find just after this list.
2. if `TRACING_KAFKA_URL` environment variable is set it logs the opentracing data on this Kafka using the topic `TRACING_KAFKA_TOPIC`. This level can be customized using `KAFKA_LOG_LEVEL`, default being `INFO`.

It uses the following format for log events:
...
This profile is very close to the previous one except it logs even the default logs - and not only the tracing ones,
on Kafka. It adds the configuration `LOG_KAFKA_TOPIC` and `LOG_KAFKA_URL` - same as the tracing flavors but for the other logs.
...
You can register component server images in docker using those instructions in the corresponding image directory :
