...
IMPORTANT: the configuration related to port `5005` (in  `ports` and the `-agentlib` option in `_JAVA_OPTIONS`)
are only useful if you want to debug the server on port `5005`, *don't* set it in *production*.
