...
=== How to integrate components into the image

==== Docker Compose

Docker Compose is a nice way to deploy the server with components. The idea is to mount the component volume
into the server image. Here is a `docker-compose.yml` doing it:
...
services:
  component-server:
    healthcheck:
      ...
      retries: 3
      test: curl --fail http://localhost:1234/api/v1/environment
    image: tacokit/component-server:${COMPONENT_SERVER_IMAGE:-1.1.5_20190110093338}
    environment:
    - _JAVA_OPTIONS=
        -Xmx1024m
        -Dhttp=1234
  ...
  connectors:
    image: ${TALEND_REGISTRY}/talend/connectors-se:${CONNECTORS:-1.0.0_master_20190110080009}
    volumes:
    - connectors:/opt/talend/connectors-se:ro
...
To run one of both compose you can use `docker-compose -f docker-compose.yml up`.
