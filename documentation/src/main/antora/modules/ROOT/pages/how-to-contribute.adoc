...
Some of the files are prefixed to halp identifying the type of content it contains. Most common examples are:
...
- `tutorial_` for tutorials/guided examples.
_ `generated_` for pages generated from the source code. These pages are generally stored in the '_partials' folder.
