...
. (optional) Update Algolia (doc search index) if you redeployed the documentation - note that it crawls the website so you can need to wait for github to redeploy the website: `cd documentation && mvn gplus:execute@groovy-scripts rmannibucau-common:filter@filter-algolia frontend:yarn@algolia-index && cd -`
