...
IMPORTANT: all commands assume the environment variable RELEASED_VERSION is set to the release you just did and you built the release locally before running the command (from the tag).
...
4. Deploy the component-server on Docker Hub: `COMPONENT_SERVER_DOCKER_BUILD_ONLY=false ./.travis/docker.sh`
