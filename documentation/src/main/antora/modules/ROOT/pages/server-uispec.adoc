...
It is possible to show under the inputs the help on each property,
to do that you must set a `PropertyContext.Configuration` instance on the `UiSpecService`
with the `includeDocumentationMetadata` attribute set to `true`.
