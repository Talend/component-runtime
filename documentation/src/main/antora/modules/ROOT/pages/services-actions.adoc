...
Internationalization is supported through the injection of the parameter `$lang` which will allow you to get the correct locale
to use with an `@Internationalized` service:
...
TIP: you can combine the `$lang` option with `@Internationalized` and `@Language` parameter.
