...
By default the client will load in memory the payload, for big payloads it can consume too much memory.
For these cases you can get the payload as an `InputStream` - using the `Response` wrapper or not:
