...
. Currently, discovered schema is read in the standard output of the process executed to infer the schema.
 It would be possible to identify error output and, with some configuration, ask to popup a dilaog bow to ask the user
 if he is agree to execute the subjob to retrieve the real schema:
. The DiscoverSchemaException is dedicated to Studio's guess schema feature.
...
.. This exception defines 5 types in HandleErrorWith:
.. EXECUTE_MOCK_JOB : Potentially execute a mock job in studio. It will ask user's validation before executing.
                      When specifying this option, developer should be sure that no side effect can be generated by connector.
.. EXECUTE_LIFECYCLE : Will execute a lifecycle through framework using only configuration. This won't query for any user input.
                       When specifying this option, developer should be sure that no side effect can be generated by connector.
.. EXCEPTION : Default case (as same as ComponentException)
.. SILENT : Unhandled
.. RETRY : Unhandled
