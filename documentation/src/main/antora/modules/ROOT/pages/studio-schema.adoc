...
. When a service fails to infer the schema, it may throw a DiscoverSchemaException.
 The outcome then depends on the HandleErrorWith setting, leading to different possible scenarios:
.. EXECUTE_MOCK_JOB: Requests permission from the user to run the job up to the selected connector.
 A pop-up will be displayed to get the authorization and prevent the user that it can generate side effects.
...
.. EXCEPTION: Display any thrown exception.
.. SILENT : Not implemented
