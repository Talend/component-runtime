...
As Talend Component Kit record's type (TODO: link to API) aren't matching Studio types, we wrap those types in wider types.
Anyway, the original type coming from Studio's IPersistentRow (TODO: link to API) descendants' type is stored in record's schema properties.
The property name is `talend.studio.type` and when handling an output connector designed for Studio, you should
have to *check* for this property to get an accurate type for output.
