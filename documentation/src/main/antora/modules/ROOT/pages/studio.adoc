...
Have a look at the https://help.talend.com/[documentation center] to check about the Dynamic Schema feature.


=== Handle the dynamic columns in your components

There's a helper class `DynamicHelper` for handling dynamic column.
    ...
    record.getSchema().getEntries().stream().forEach(entry -> {
            if (DynamicColumnsHelper.isDynamicColumn(entry.getName())) {
                ...
                dynamicRecord.getSchema().getEntries().stream().forEach(e -> {
                        json.put(e.getName(), jsonValueFromRecordValue(e, dynamicRecord));
                    });
            } else {
                json.put(entry.getName(), jsonValueFromRecordValue(entry, record));
            }
        });
    processJson(json);
}
...
public Record next() {
   ...
   if (DynamicHelper.hasDynamicColumn(columnsSet)) {
