...
. In the *Input(s) / Output(s), click the default *MAIN* input branch to access its detail, and toggle the *Generic* option to specify that the component can receive any type of data. Leave the *Name* of the branch with its default `MAIN` value. +
+
...
At this point, your component is technically already ready to be compiled and deployed to Talend Open Studio. But let's first have a look at the generated project:
...
Without any modification in the component code, let's compile the project and deploy the component to a local instance of Talend Open Studio.
