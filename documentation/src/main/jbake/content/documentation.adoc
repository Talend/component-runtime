...
use `ObjectMap` as parameter type and Talend Component will just wrap the data to enable you to access it as a map. The parameter
...
component reusable in any chain it is more than highly encouraged to use `ObjectMap` until you have your an evaluation language
based processor (which has its own way to access component). Here is an example:

[source,java]
----
@ElementListener
public MyNewData map(final ObjectMap incomingData) {
    String name = String.class.cast(incomingData.get("name"));
    int name = Integer.class.cast(incomingData.get("age"));
    return ...;
}
...
==== ObjectMap and dynamic schemas

If you don't know at coding time the structure of your object you can use `@ObjectMap.Any` on a `Map<String, Object>` field
which means the `ObjectMap` mapping will use the (unique) decorated field to find any entry in the object.
...
the framework will be able to access any column:

[source,java]
----
final Object name = jdbcRecordObjectMap.get("name");
----
...
These metadata are set on the component class and belongs to `org.talend.component.api.component` package.
...
<3> you can use `@Language` on a `Locale` parameter to specify manually the locale to use, note that a single value will be used (the first parameter taggeg as such).
...
The following files have been resolved:
   org.talend.components:component-api:jar:1.0.0-SNAPSHOT:provided
...
- the API (`component-api` and `geronimo-annotation_1.0_spec`) are `provided` because you can consider them to be there when executing (it comes with the framework)
...
<plugin>
  ...
  <executions>
    <execution>
      ...
      <configuration>
        <transformers>
          <transformer implementation="org.talend.component.container.maven.shade.ContainerDependenciesTransformer">
...
<plugin>
  ...
  <executions>
    <execution>
      ...
      <configuration>
        <transformers>
          <transformer implementation="org.talend.component.container.maven.shade.PluginTransformer">
            <session>${session}</project>
          </transformer>
        </transformers>
      </configuration>
  ...
  <executions>
    <execution>
      ...
      <configuration>
        ...
        <transformers>
          <transformer
              implementation="org.talend.component.container.maven.shade.ContainerDependenciesTransformer">
...
<transformer implementation="org.talend.component.container.maven.shade.PluginTransformer">
  <session>${session}</session>
  <include>org.talend.components:component-x,org.talend.components:component-y,org.talend.components:component-z</include>
</transformer>
----
...
validation in `org.talend.component.api.configuration.constraint` package:
...
`org.talend.component.runtime.beam.TalendIO` provides a way to convert a partition mapper or a processor to an input or processor
using the `read` or `write` methods.

==== Processors

`org.talend.component.runtime.beam.TalendFn` provides the way to wrap a processor in a Beam `PTransform` and integrate it in the pipeline.
...
The first goal is here to help you to validate the common programming model of the component. Here is the execution definition to activate it:
...
The `prepare` goal is to build artifacts you can deploy out of the box into the studio. It is a temporarly phase
until the framework programming model is fully integrated in Talend suite.
...
<plugin>
  ...
  <executions>
    <execution>
      <id>talend-component-prepare</id>
      <goals>
        <goal>prepare</goal>
      </goals>
    </execution>
...
It will generate a maven project in `${project.build.directory}/oldtcomponentbridgemojo_workdir` which contains the artifacts
you can deploy to the studio. Here is the associated configuration:

[options="header,autowidth"]
|===
|Name|Description|Default
|attach|attach the built virtual artifacts (definition and runtime modules) to the build/project (enables to deploy them)|true
|componentNgApiVersion|The component API version, if set to `auto` uses the one of the plugin (recommanded)|auto
|componentsApiVersion|The bridge component API version, if set to `auto` uses the one of the plugin (recommanded)|auto
|studioHome|If set and `attach` is true, the created component will be installed in the studio located at that path|null
|===

TIP: if you configure a `studio` profile in your ~/.m2/settings.xml, you can activate/deactivate the deployment into the studio of your component
with a profile (adding `-Pstudio` to you build command, ex: `mvn clean install -Pstudio`). Here is a sample declaration of such a profile:

[source,xml]
----
<profile> <!-- to deploy in the studio at build time: mvn clean install -Pstudio -->
  <id>studio</id>
  <properties>
    <talend.component.studioHome>/opt/talend/TOS_DI-V6.5.0</talend.component.studioHome>
  </properties>
</profile>
----
