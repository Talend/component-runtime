...
|${class_name}.${property_name}._displayName|the display name of the option using it class name.
|====
...
Configuration class are also translatable using the class name in the messages properties file.
...
You can give it a translatable display name by adding ${class_name}.${property_name}._displayName to Messages.properties under the same package as the config class.
[source]
----
...
IMPORTANT: if you have a display name using the property path, it will surcharge the display name defined using the class name
...
IMPORTANT: this goal is deprecated.
