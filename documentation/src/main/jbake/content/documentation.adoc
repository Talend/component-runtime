...
IMPORTANT: in distributed environments class' constructor will be called on driver node, methods annotated with
...
2. During partition plan computation step pipeline is analyzed and split into stages. Driver(Manager) node instantiates mappers/processors
...
pipeline execution is performed. After pipeline is processed, methods annotated with @PreDestroy annotation are called.
....

[ditaa, driver-processing-workflow, png]
....
...
[ditaa, worker-processing-workflow, png]
....
Flow Execution(3)
+------------------+
|  @PostConstruct  |
