...
|--------------|    |--------------|    |--------------|
|Flow Execution|    |Flow Execution|    |Flow Execution|
+--------------+    +--------------+    +--------------+
....
...
IMPORTANT: After splitting mappers at main cluster node, splitted mappers are serialized to secondary(worker) nodes, where the
pipeline process is performed.
