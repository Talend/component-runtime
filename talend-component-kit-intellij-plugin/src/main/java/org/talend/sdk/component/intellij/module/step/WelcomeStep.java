...
public class WelcomeStep extends ModuleWizardStep {
    ...
    public WelcomeStep(WizardContext context, Disposable parentDisposable) {
    ...
    public static final class WelcomePanel extends JPanel {
        ...
        public WelcomePanel() {
            ...
            try (InputStream is = Objects.requireNonNull(getClass().getClassLoader()
                                                                   .getResourceAsStream("/talend_logo_.png"));) {
            ...
            titleLabel.setFont(new Font(titleLabel.getFont()
                                                  .getName(), Font.BOLD, 24));
            headerPanel.add(titleLabel);

            try (InputStream is = Objects.requireNonNull(getClass().getClassLoader()
                                                                   .getResourceAsStream("/tacokit_logo.png"));) {
                headerPanel.add(new JLabel(new ImageIcon(ImageIO.read(is))));
            } catch (IOException e) {//no-op
            }
